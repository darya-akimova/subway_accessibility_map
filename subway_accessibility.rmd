...
```{r}
tract.cent <- st_centroid(nyc.map.4boro)
plot(st_geometry(tract.cent), pch = 16, cex = 0.25)
test <- st_buffer(tract.cent, dist = 2500)
plot(st_geometry(test))
plot(st_geometry(nyc.map.4boro), col = "grey", border = "white")
plot(test, col = "#D55E00", add = TRUE)

plot(st_geometry(test %>% filter(BoroCT2010 == 1009800 | BoroCT2010 == 1018400 )))

test2 <- st_join(sub.sf.nyc.crs, test)
test3 <- st_set_geometry(test2, NULL)
