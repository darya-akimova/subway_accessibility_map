...
```{r comment=NA}
sub.cln.sf <- st_as_sf(sub.stat.final, coords = c("avg_stat_long", "avg_stat_lat"), crs = 4326)
...
nyc.neigh.4boro %>% 
  ...
  geom_sf(
    ...
    size = 1.5, aes(color = primary_trunk_line)
    ) +
  scale_color_manual(
...
stat.by.route %>% 
  ...
  # total number of station stops on route
  geom_point(size = 3, color = "#0072B2") +
  # number of ada-accessible station stops on route
  geom_point(aes(total_stat_order, num_ada), size = 3, color = "#56B4E9") +
  xlab("Route Name") +
  ylab("Number of Stations") +
  ggtitle("Total number of stations vs number of ADA-accessible stations") +
  coord_flip()
...
boro.stat.count %>% 
  ggplot(aes(boroname, tot_stat, fill = boroname)) + 
  geom_bar(stat= "identity") +
  scale_fill_tableau() +
  xlab("Borough Name") +
  ylab("Percent of Stations that are ADA") +
