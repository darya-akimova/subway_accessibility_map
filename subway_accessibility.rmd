...
* Some station ADA-accessibility ratings have been updated from `FALSE` to `TRUE`, all of which had no indication that changes were under construction and were not mentioned in the `ada_notes` column.
...
The summary is prepped, so it's time to plot, starting with a bar graph of the total number and the number of ADA accessible subway station stops by route. The bars will be colored by their traditional NYC subway colors.
...
The number of ADA-accessible stations by route is far less than the total number of stations, which is not unexpected. Let's take a look at the percent of subway stations that are ADA-accessible per route.
...
Out of the 25 subway lines, which are worse or better than expected?
...
10 out of the 25 routes have fewer than 25% ADA-accessible station stops, with the N, J, Z, and G suffering from particularly poor accessibility. Only 2 trains, one of which is a shuttle train with 4 total stops, make it above 50% mark. 
...
What is missing for this section is a borough assignment for each station. While one option is to find another dataset that matches subway stations to a borough or to try and create complex rules to assign stations by latitude and longitude, instead I can take advantage of the spatial join `st_join()` function from the `sf` package. Because subway dataset and the NYC maps now have the same CRS, a spatial join can be used to merge the data based on geometry.
...
boro.stat.count %>% 
  ...
  ggtitle("Subway station stop count by borough")
```


Perhaps unsurprisingly, Manhattan has the highest count of subway stations, followed by Brooklyn, then Queens and lastly the Bronx. Even though more people live in Brooklyn and Queens, Manhattan is, to some extent, the tourist and economic hub of the city where many people visit and work. One reason for this is the way that I have chosen to count subway stations. By counting each route stop individually, even if they stop at the same physical subway station, the number becomes very high because all subway station lines, with the exception of 2 shuttles and the G, pass through Manhattan and make many stops, typically going along the length of the island and making may stops. 
...
nyc.neigh.count.prep %>% 
  ...
  # overlay subway:
  geom_sf(data = sub.sf.nyc.crs, color = "grey90", fill = "black", size = 1.5, pch = 21) +
...
nyc.4boro.buff.join %>% 
  ...
  scale_fill_manual(values = c("#c7e9b4", "#7fcdbb", "#41b6c4", "#2c7fb8", "#253494", "#b8652c", "grey30"), na.value = "gray50", name = "Percent ADA") +
  geom_sf(data = sub.sf.nyc.crs, color = "grey90", fill = "black", size = 1.5, pch = 21) +
  geom_sf(data = sub.sf.nyc.crs %>% filter(stat_name == "bmt_4 avenue_95th st"), color = "white", size = 2) +
  geom_sf(data = sub.sf.nyc.crs %>% filter(stat_name == "irt_dyre av_eastchester-dyre av"), color = "yellow", size = 3) +
  geom_sf(data = sub.sf.nyc.crs %>% filter(stat_name == "irt_new lots_new lots av"), color = "yellow", size = 2, shape = 15) +
  theme(axis.text.x = element_blank(), axis.text.y = element_blank())
```
