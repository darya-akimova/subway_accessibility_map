...
* Some stations seem to be listed twice because different trunk line train stop there. For example, again, Union Square, which is considered one station, is listed as `bmt_broadway_union square` for the N/Q/R/W trunk line and `bmt_canarsie_union square` for the L trunk line. These duplicates will need to be removed for an accurate count of train routes down the line.
...
Solution 1: Import [a dataset form the NYC Open Data website with updated station coordinates](https://data.cityofnewyork.us/Transportation/Subway-Stations/arq3-7z49) and try to merge the ADA data with the new station information.
...
Could quickly compare the [population estimates per borough (2017 estimates from Wikipedia)](https://en.wikipedia.org/wiki/Demographics_of_New_York_City) to the number of subway stops. 

```{r boro_pop_comp, comment=NA}
...
table(nyc.neigh.count.prep$`Neighborhood Type`)
# Map:
nyc.neigh.count.prep %>% 
  ggplot() +
  geom_sf(aes(fill = `Neighborhood Type`), color = "#BFBFBF") +
...
```{r neigh_count_plots, comment=NA}
...
```{r neigh_percent_ada_plots, comment=NA}
...
nyc.neigh.count.prep %>% 
  ...
  mapview(zcol = "Percent ADA-Accessible", col.regions = c("#c7e9b4", "#7fcdbb", "#41b6c4", "#2c7fb8", "#253494", "#E69F00", "gray50", "grey20"))
```
...
```{r buff_join_tot_plot, comment=NA}
nyc.4boro.buff.join %>% 
  mutate(tot_fact = cut(tot_stat, quantile(tot_stat, na.rm = TRUE), include.lowest = TRUE)) %>% 
  ggplot() +
  geom_sf(aes(fill = tot_fact), color = "black") +
...
```{r no_sub_neigh_map, comment=NA}
nyc.neigh.count.prep %>% 
  ggplot() +
  geom_sf(aes(fill = `Neighborhood Type`), color = "black") +
  scale_fill_manual(values = c("grey30", "#0072B2", "#E69F00", "#999999", "#bae4b3")) + 
  theme(axis.text.x = element_blank(), axis.text.y = element_blank()) +
  ggtitle("Neighborhoods with no accessible subway station")
```
...
nyc.4boro.buff.join %>% 
  ...
  {table(.$color_group, .$`Neighborhood Type`)}
```
...
nyc.4boro.buff.join %>% 
  ...
  mutate(
    group_diff = case_when(
      color_group == "Park" ~ "Park",
      color_group == "Airport" ~ "Airport",
      color_group == `Neighborhood Type` ~ "Same",
      TRUE ~ "Different"
    )
  ) %>% 
  ggplot() +
...
access.diff <- nyc.4boro.buff.join %>% 
  ...
  mutate(
    access_diff = case_when(
      color_group == "Park" ~ "Park",
      color_group == "Airport" ~ "Airport",
      color_group == "At least one accessible station" & `Neighborhood Type` == "At least one accessible station" ~ "Both Accessible",
      color_group != "At least one accessible station" & `Neighborhood Type` == "At least one accessible station" ~ "Neigh Access / Tract No",
      color_group == "At least one accessible station" & `Neighborhood Type` != "At least one accessible station" ~ "Tract Access / Neigh No",
      TRUE ~ "Other"
    )
  ) 
